...
const Ticket = () => {
  ...
  useEffect(() => {
    const fetchData = async () =>{
        try{
            const {data: ticketDataTemp} = await api.ticket.getTicketById(router.query.ticketId);
  ...
  const validateTicket = async (e) =>{
      ...
      try{
          ...
          const {data:ticketDataTemp} = await api.ticket.updateTicket(router.query.ticketId, data);
